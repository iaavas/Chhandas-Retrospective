...
export function detectSyllablesWithMapping(text: string): SyllableAnalysis {
  ...
  for (let ti = 0; ti < tokens.length; ti++) {
    ...
    if (token.length === 1 && independentVowelRe.test(token)) {
      const ch = token;
      if (INDEPENDENT_LONG.has(ch)) isGuru = true;
      else if (INDEPENDENT_SHORT.has(ch)) isGuru = false;
      else isGuru = false;
    }
...
export function detectAnustubh(text: string): AnustubhResult {
  ...
  if (rawLines.length === 4) inputFormat = "4-line";
  else if (rawLines.length <= 2) inputFormat = "2-line";
  ...
  let syllableIndex = 0;
  const aksharaToSyllableMap: number[] = [];
...
export function processStanza(text: string): StanzaResult {
  const lines = text
    .trim()
    .split("\n")
    .filter((line) => line.trim());
