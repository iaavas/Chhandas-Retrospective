/**
 * Chhandas MCP Server factory
 *
 * Creates and configures the MCP Server with all tool definitions and handlers.
 * Separated from transport concerns so it can be used with both stdio (local)
 * and HTTP (Vercel) transports.
 */
...
export function createChhandasServer(): Server {
        ...
        inputSchema: {
          ...
          properties: {
            classification: {
              ...
              description: "Filter by classification. Defaults to 'all'.",
            },
          },
          required: [],
        },
    ...
    switch (name) {
      ...
      case "analyze_meter": {
        ...
        const lines = results.map((r) => {
          ...
          const aksharaLabels = r.aksharas
            .map((a, i) => {
              ...
              const weight =
                sIdx !== null && sIdx !== undefined ? r.syllables[sIdx] : "·";
              return `${a}(${weight})`;
            })
            .join(" ");
        ...
        if (
          ...
          (anustubhResult.totalSyllables >= 28 &&
            anustubhResult.totalSyllables <= 36)
        ) {
          sections.push("");
          sections.push("--- Anustubh (Shloka) Analysis ---");
          sections.push(
            `  Result: ${anustubhResult.isAnustubh ? "✓ IS Anustubh" : "✗ NOT Anustubh"}`
          );
          sections.push(`  Confidence: ${anustubhResult.confidence}%`);
          sections.push(
            `  Total syllables: ${anustubhResult.totalSyllables} (expected 32)`
          );
          sections.push(`  Input format: ${anustubhResult.inputFormat}`);

          if (anustubhResult.overallErrors.length > 0) {
            sections.push("  Issues:");
            anustubhResult.overallErrors.forEach((e) =>
              sections.push(`    • ${e}`)
            );
          }
      ...
      case "list_meters": {
        ...
        for (const [meter, pattern] of Object.entries(CHHANDAS)) {
          ...
          const patternStr =
            pattern.length > 0 ? pattern.join(" ") : "(special rules)";
